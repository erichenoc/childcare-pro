...
export default function EditClassroomPage() {
  ...
  async function loadClassroom() {
    try {
      setIsLoading(true)
      const classroom = await classroomsService.getById(classroomId)

      if (classroom) {
        setFormData({
          name: classroom.name,
          age_group: classroom.age_group || 'threes',
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      await classroomsService.update(classroomId, {
        name: formData.name,
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Editar Salon</h1>
          <p className="text-gray-500">Actualizar informacion del salon</p>
        </div>
      </div>
